...
class LorcanaJsonParser:
    ...
    def get_card_by_fullname(self, fullname: str) -> Optional[Dict[str, Any]]:
        """Get a specific card by its full name"""
        for card in self.cards:
            if card.get('fullName') == fullname:
                return card
        return None
    
    def get_cards_by_fullnames(self, fullnames: List[str]) -> List[Dict[str, Any]]:
        """Get multiple cards by their full names"""
        result = []
        for fullname in fullnames:
            card = self.get_card_by_fullname(fullname)
            if card:
                result.append(card)
        return result
